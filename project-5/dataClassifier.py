...
def enhancedPacmanFeatures(state, action):
    ...
    for food in foodList:
        distanceToFood.append(util.manhattanDistance(food, pacmanPosition))
    if len(distanceToFood) != 0:
        nearestFood = min(distanceToFood)
        features["nearest_food"] = 1.00 / nearestFood   
    distanceToGhost = []
    ghosts = successorState.getGhostPositions()
    for ghost in ghosts:
        distanceToGhost.append(util.manhattanDistance(ghost, pacmanPosition))
    if len(distanceToGhost) != 0:
        features["nearest_ghost"] = min(distanceToGhost)
    return features
